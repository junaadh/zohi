...
b8 platform_startup(
    ...
    if (state->display == NULL) {
        ZFATAL("Failed to open X display. \nCheck if $DISPLAY is set correctly. \nif not set `export $DISPLAY=:0` and rerun");
        return FALSE;
    }
    ...
    if (xcb_connection_has_error(state->connection)) {
        ZFATAL("Failed to connect to X server via XCB");
        return FALSE;
    }
    ...
    // after looping assign
    state->screen = it.data;

    // allocate xid for the window
    ...
    if (stream_result <= 0) {
        ZFATAL("An error occurred when flusing the stream: %d", stream_result);
        return FALSE;
    }
