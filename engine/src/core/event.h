 ...
 * @param on_event the callback function pointer to be invoked whe the event code is fired
 * @returns TRUE if the event is successfully registered. FALSE otherwise
 */
ZAPI b8 event_register(u16 code, void *listener, PFN_on_event on_event);
 ...
 */
ZAPI b8 event_unregister(u16 code, void *listener, PFN_on_event on_event);
 ...
 */
ZAPI b8 event_fire(u16 code, void *sender, event_context context);
